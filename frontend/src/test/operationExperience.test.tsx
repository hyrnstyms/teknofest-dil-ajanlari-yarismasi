...
const item: CaseRecord = {
  ...
  clarification: { needs_clarification: true, blocking: true, requested_fields: ["address"], question_type: "free_text", question: "Olay adresi nedir?", options: [], resume_target: "READY_TO_ROUTE" },
  timeline: [], department_actions: [], drafts: [{ id: "d1", draft_type: "MISSING_INFORMATION_REQUEST", draft_status: "DRAFT", subject: "Adres bilgisi talebi", recipient: "Başvuru Sahibi", body: "Adres bilgisini iletiniz.", ai_generated: true }],
...
describe("EVRAG operation-first experience", () => {
  ...
  it("presents source, route, empty assignment and no invented personnel", () => {
    const onRoute = vi.fn();
    render(<CaseOperationPlan item={item} onRoute={onRoute}/>);
    expect(screen.getByText("EVRAG AKILLI İŞLEM PLANI")).toBeInTheDocument();
    expect(screen.getByText("Henüz oluşturulmadı")).toBeInTheDocument();
    expect(screen.getByText(/Gerçek konum verisi bulunmuyor/)).toBeInTheDocument();
    expect(screen.queryByText(/Ahmet|Mehmet|Ayşe/)).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: /FEN İŞLERİ MÜDÜRLÜĞÜ BİRİMİNE HAVALE ET/ }));
  ...
  it("explains missing information and chooses the correct source", () => {
    ...
    expect(screen.getByRole("button", { name: "Bilgi Talebini Oluştur" })).toBeEnabled();
  });
  ...
  it("offers exactly the actionable next step and explains draft grounding", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /Bilgi talebi oluştur/ }));
    expect(onAction).toHaveBeenCalledWith("clarification");
    expect(screen.getByText("BU TASLAK NEDEN ÜRETİLDİ?")).toBeInTheDocument();
    expect(screen.getByText("Dosya analizi ve insan kontrolü")).toBeInTheDocument();
  });
});
