...
function App() {
  return (
    <BrowserRouter>
      <AuthProvider>
        <Routes>
          <Route path="/takip/:trackingCode" element={<CitizenTracePage />} />
          <Route path="/dogrulama/:id" element={<QrVerifyPage />} />
...
function AuthenticatedShell() {
  ...
  if (loading) return <div className="app-boot">EVRAG güvenli oturumu doğrulanıyor…</div>;
  if (!user) return <DemoLoginPage />;
...
function WorkspaceShell() {
  ...
  const legacyMatch = useMatch("/evrak/:id");
  const caseMatch = useMatch("/dosya/:id");
  ...
  const institution = useMemo<InstitutionOption>(() => ({
    id: user!.institution_id,
    label: user!.institution_id === "belediye" ? "Belediye" : "Kaymakamlık",
    ui_config: { description: user!.department_name || user!.department_code },
  }), [user]);
  ...
  const chatAnalysisId = legacyMatch?.params.id === activeAnalysisId ? activeAnalysisId : undefined;

  return (
    <>
      <div className="app-layout">
        <Sidebar role={user!.role} onOpenCopilot={() => setCopilotOpenSignal((value) => value + 1)} />
        <div className="app-main">
          ...
          <main className="main-content">
            <Routes>
              ...
              <Route path="/ai-operasyon" element={<AIOperationsPage institution={institution} onOpenAnalysis={(id) => navigate(`/evrak/${id}`)} />} />
      ...
      <ChatWidget
        ...
        caseId={caseMatch?.params.id}
        currentDraft={chatAnalysisId ? activeDraft : undefined}
