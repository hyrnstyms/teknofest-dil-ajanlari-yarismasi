...
export function OfficialWritingWorkspace({ item, token, onRefresh, onNotice }: { item: CaseRecord; token: string; onRefresh: () => Promise<void>; onNotice: (message: string) => void }) {
  ...
  return <section className="case-panel official-writing-workspace">
    <header><div><span className="eyebrow">TEKNOFEST GÖREV 2</span><h2>Resmî Yazılar <b>({drafts.length})</b></h2><p>Yazı türü, doğru muhatap, personel düzeltmesi ve onay aynı vaka kaydında izlenir.</p></div></header>
    ...
    {!drafts.length && <div className="writing-locked"><LockKeyhole/><div><strong>Resmî yazı alanı hazır</strong><p>{lockedMessage}</p><span>Kurum İşlemi → Taslak → Düzenleme → Onay → Çıktı</span></div></div>}
