...
function CaseLifecycle({ item }: { item: CaseRecord }) {
  ...
  const approvedResponse = item.drafts.some((draft) => isResponseDraft(draft) && draft.draft_status === "APPROVED");
  const stateFor = (index: number) => index === 4 && approvedResponse && current < 5 ? "done" : index < current ? "done" : index === current ? "current" : "future";
  return <section className="case-lifecycle" aria-label="Dosya ilerleme durumu">
    {stages.map((stage, index) => <React.Fragment key={stage}>
      <span className={stateFor(index)}><i>{stateFor(index) === "done" ? <Check/> : index + 1}</i>{stage}</span>
...
export function CaseProductPanels({ item, token, onRefresh, onNotice, activeTab, onTabChange, actionPanel }: Props) {
  ...
  const responseDrafts = item.drafts.filter(isResponseDraft);
  const responseItem = { ...item, drafts: responseDrafts };
  ...
  return <>
    ...
    <nav className="case-workspace-tabs" aria-label="Dosya çalışma alanı bölümleri">
      ...
      <button className={tab === "writings" ? "active" : ""} onClick={() => selectTab("writings")}>Cevap Taslağı{responseDrafts.length ? ` (${responseDrafts.length})` : ""}</button>
    ...
    <div className="case-tab-content">
      ...
      {tab === "writings" && <><WritingGroundingSummary item={responseItem} onInspect={() => document.getElementById("official-writing-detail")?.scrollIntoView({ behavior: "smooth", block: "start" })}/><OfficialWritingWorkspace item={responseItem} token={token} onRefresh={onRefresh} onNotice={onNotice}/></>}
