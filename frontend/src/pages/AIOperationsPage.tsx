...
import { Activity, AlertTriangle, BookOpen, Bot, Building2, CheckCircle2, Cpu, Database, FileSearch, Gauge, Network, RefreshCw, Server, ShieldCheck, Workflow } from "lucide-react";
...
export const AIOperationsPage: React.FC<Props> = ({ institution, onOpenAnalysis }) => {
  ...
  return <div className="aiops-page no-print">
    <header className="aiops-hero">
      <div><span className="section-kicker">Teknik görünüm · gerçek runtime verisi</span><h1>AI Operasyon Merkezi</h1><p>Çok ajanlı evrak işleme, yerel yapay zekâ altyapısı, RAG kanıtları ve son analiz performansı.</p></div>
      <button className="btn btn-secondary" type="button" onClick={() => void load()} disabled={loading}><RefreshCw size={16} /> {loading ? "Yenileniyor" : "Verileri Yenile"}</button>
    </header>
    ...
    {!analysis ? <section className="aiops-empty"><Workflow size={34} /><h2>Henüz analiz edilmiş evrak bulunmuyor</h2><p>Yeni Evrak ekranından bir evrak analiz ederek AI sürecini burada inceleyebilirsiniz.</p></section> : <>
      <div className="aiops-primary-grid">
        <section className="aiops-panel">
          <SectionTitle icon={<Workflow />} eyebrow="Son analizin gözlemlenebilir kaydı" title="Çok Ajanlı İşlem Akışı" />
          <div className="aiops-flow">{stages.map(([id, label, description], index) => {
            ...
            return <div className={"aiops-node " + (complete ? "complete" : "unknown")} key={id}>
              <span className="aiops-node-index">{complete ? <CheckCircle2 size={17} /> : index + 1}</span>
              <div><strong>{label}</strong><small>{description}</small></div>
              <time>{normalized?.duration_ms !== undefined ? formatDuration(normalized.duration_ms) : "Süre kaydı yok"}</time>
            </div>;
          })}</div>
        </section>

        <section className="aiops-panel aiops-report">
          <SectionTitle icon={<FileSearch />} eyebrow="Backend analysis detail" title="Son Analiz Raporu" />
          <dl>
          ...
          {analysis.analysis_id && <button className="btn btn-primary" type="button" onClick={() => void onOpenAnalysis(analysis.analysis_id!)}>Evrakı Aç</button>}
        </section>
      </div>

      <section className="aiops-panel">
        <SectionTitle icon={<Activity />} eyebrow="Gerçek node_timings" title="Performans ve Süre Dağılımı" />
        <div className="aiops-bars">{stages.map(([id, label]) => {
          const value = timings[id];
          const duration = typeof value === "number" ? value : value?.duration_ms;
          if (typeof duration !== "number") return null;
          return <div className="aiops-bar-row" key={id}><span>{label}</span><div><i style={{ width: Math.max(1, duration / maxTiming * 100) + "%" }} /></div><strong>{formatDuration(duration)}</strong></div>;
        })}</div>
      </section>
      ...
      <div className="aiops-secondary-grid">
        <section className="aiops-panel">
          ...
          <p className="aiops-note">Kurum değişikliği merkezi üst çubuktan yapılır; değişiklik eski aktif evrak ve Copilot bağlamını temizler.</p>
        </section>

        <section className="aiops-panel">
          <SectionTitle icon={<BookOpen />} eyebrow="Analizde kullanılan mevzuat" title="RAG Kanıt Denetçisi" />
          {evidence.length ? <div className="aiops-evidence">{evidence.map((entry: any, index: number) => {
            ...
            return <article key={index}><strong>{source.title || source.document_id || entry.law_name || "Mevzuat kaynağı"}</strong><span>{source.law_number ? source.law_number + " sayılı" : "Kanun numarası sağlanmıyor"} {source.madde_no || source.article ? " · Madde " + (source.madde_no || source.article) : ""}</span><p>{entry.evidence || entry.text || source.text}</p><small>{source.source || entry.source}</small></article>;
          })}</div> : <div className="aiops-inline-empty">Bu analizde doğrulanmış mevzuat kanıtı bulunmuyor. Retrieval adayları kanıt olarak gösterilmez.</div>}
        </section>
      </div>
    </>}
    ...
    <div className="aiops-secondary-grid">
      <section className="aiops-panel">
        ...
        <div className="aiops-tech-grid">
          <Tech label="Üretken model" value={system?.llm_model || "Runtime verisi yok"} />
          <Tech label="Provider" value={system?.llm_provider || "Runtime verisi yok"} />
          ...
          <Tech label="Uygulama" value="React + Vite · FastAPI" />
          <Tech label="Çalışma biçimi" value={system?.llm_provider === "ollama" ? "Yerel / Offline" : "Provider yapılandırmasına bağlı"} />
        </div>
        <div className="aiops-architecture"><span>React UI</span><b>→</b><span>FastAPI</span><b>→</b><span>Multi-Agent Workflow</span><b>→</b><span>Human Review</span><b>→</b><span>DOCX</span></div>
      </section>

      <section className="aiops-panel">
        <SectionTitle icon={<Gauge />} eyebrow="Backend sözleşmesi görünürlüğü" title="Sistem Telemetrisi" />
        <div className="aiops-telemetry">
          ...
          <Telemetry label="Embedding boyutu" value={system?.embedding_dimension} available={Boolean(system?.embedding_dimension)} />
          <Telemetry label="GPU / VRAM / sıcaklık" value="Backend telemetry endpointi mevcut değil" available={false} />
          <Telemetry label="OCR readiness" value="Ayrı readiness alanı sağlanmıyor" available={false} />
          <Telemetry label="Anlık agent aşaması" value="Intermediate progress endpointi sağlanmıyor" available={false} />
        </div>
      </section>
    </div>
  </div>;
...
const SectionTitle: React.FC<{ icon: React.ReactNode; eyebrow: string; title: string }> = ({ icon, eyebrow, title }) => <header className="aiops-section-title"><span>{icon}</span><div><small>{eyebrow}</small><h2>{title}</h2></div></header>;
const StatusCard: React.FC<{ icon: React.ReactNode; label: string; value: string; ok: boolean; detail?: string }> = ({ icon, label, value, ok, detail }) => <article className="aiops-status-card"><span className={"aiops-status-icon " + (ok ? "ok" : "warn")}>{icon}</span><div><small>{label}</small><strong>{value}</strong><p>{detail || "Ek veri sağlanmıyor"}</p></div></article>;
const AgentCard: React.FC<{ title: string; task: string; result: string; timing?: number }> = ({ title, task, result, timing }) => <article className="aiops-agent-card"><span className="aiops-agent-dot" /><h3>{title}</h3><small>Görev</small><p>{task}</p><small>Son sonuç</small><strong>{result}</strong><time>{timing === undefined ? "Süre kaydı yok" : formatDuration(timing)}</time></article>;
const Metric: React.FC<{ label: string; value?: string | null }> = ({ label, value }) => <div><dt>{label}</dt><dd>{value || "Veri sağlanmıyor"}</dd></div>;
const Tech: React.FC<{ label: string; value: string }> = ({ label, value }) => <div><small>{label}</small><strong>{value}</strong></div>;
const Telemetry: React.FC<{ label: string; value: string | number | undefined; available: boolean }> = ({ label, value, available }) => <div><span className={available ? "available" : "missing"}>{available ? "AVAILABLE" : "MISSING"}</span><strong>{label}</strong><p>{value ?? "Veri sağlanmıyor"}</p></div>;

function durationOf(value: number | { duration_ms?: number } | undefined): number | undefined { return typeof value === "number" ? value : value?.duration_ms; }
function formatDuration(ms: number): string { return ms >= 1000 ? (ms / 1000).toLocaleString("tr-TR", { maximumFractionDigits: 2 }) + " sn" : ms + " ms"; }
function humanize(value?: string | null): string { return value ? value.replaceAll("_", " ").replace(/w/g, (letter) => letter.toLocaleUpperCase("tr-TR")) : "Veri sağlanmıyor"; }