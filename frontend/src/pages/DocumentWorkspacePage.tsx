  ...
  return (
    <div className="page-container product-workspace">
      ...
      {workspaceTab === 'draft' && <div className="workspace-draft-layout">
        ...
        <aside className="workspace-draft-side"><RoutingCard routing={state.routing}/><QualityFormatCard quality={state.quality}/>{!reviewOpen && state.human_review?.status !== 'approved' && <button className="btn btn-primary" type="button" onClick={() => setReviewOpen(true)}>İncelemeyi Tamamla</button>}{(reviewOpen || state.human_review?.status === 'approved') && <div className="review-sheet"><header><h2>Personel İncelemesi</h2>{reviewOpen && <button type="button" onClick={() => setReviewOpen(false)}>×</button>}</header><HumanReviewPanel review={state.human_review} analysisId={state.analysis_id || state.document_id} onUpdate={async () => { await handleUpdate(); setReviewOpen(false); }} onEdit={() => { startEdit(); setReviewOpen(false); }} transferRouting={state.transfer_routing}/></div>}</aside>
      </div>}
        </div>
      </div>
    </div>
